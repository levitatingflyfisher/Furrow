...
/// Goldens must not rot with the wall clock: TodayScreen reads clock.now()
/// (package:clock), so the week strip's highlighted column and the virtue-
/// of-the-week banner are pinned here to a fixed Wednesday. Before this pin
/// the suite failed on any day but the one the goldens were generated on.
final _pinnedNow = DateTime(2026, 7, 15, 12); // Wednesday, noon (DST-safe)

void main() {
  testWidgets('Today grid — three cadences + virtue, swept sizes/text-scale', (
    tester,
  ) async {
    await withClock(Clock.fixed(_pinnedNow), () async {
      ...
      for (final i in [0, 1, 2, 3]) {
        await repo.setBinary(
          walk,
          monday.add(Duration(days: i)).toDateDay(),
          true,
        );
      }
      ...
      await repo.adjustCount(
        (await repo.getHabit(waterId))!,
        today.toDateDay(),
        5,
      );
      ...
      final tempId = await repo.createHabit(
        name: 'Temperance',
        cadence: Cadence.binary,
        virtueKey: 'temperance',
      );
      await repo.setBinary(
        (await repo.getHabit(tempId))!,
        today.toDateDay(),
        true,
      );
