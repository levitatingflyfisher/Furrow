...
/// Furrow's own presentation of the encrypted-backup flow — the same
/// [BackupController] / [SeedPhraseModal] / [PhraseEntryDialog] state machine
/// every sanctuary app uses, laid out in Furrow's own `ListTile` +
/// `LucideIcons` convention rather than the package's generic (Material-icon)
/// `BackupSettingsSection`, which would visually clash with the rest of this
/// screen (SANCTUARY-BRIEF §4.W2 — the same choice Sundial's wiring made).
///
/// The state machine itself is not reinvented: every tile's `onTap` calls
/// straight into [BackupFlow] (`sanctuary_backup_ui`'s reusable orchestration
/// helper), so the file-pick → destructive-confirm → wrong-phrase fallback →
/// outcome flow — and its config-driven dialog copy (confirmTitle /
/// confirmActionLabel) and large-text-scale-safe scrollable AlertDialogs —
/// stays a single tested path shared with `BackupSettingsSection` (W4:
/// flow-helper adoption, replacing a ~130-line copy of this same logic).
